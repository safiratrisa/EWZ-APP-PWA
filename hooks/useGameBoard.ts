...
import { clamp, createIndexArray, nextTileIndex, getId, resetTileIndex, shuffle, create2DArray } from "../utils/common";
...
const getEmptyCellsLocation = (grid: Cell[][]) =>
  grid.flatMap((row, r) => row.flatMap<Location>((cell, c) => (cell == null ? { r, c } : [])));

const createNewTilesInEmptyCells = (emptyCells: Location[], tilesNumber: number) => {
  const actualTilesNumber = emptyCells.length < tilesNumber ? emptyCells.length : tilesNumber;
...
const sortTiles = (tiles: Tile[]) => tiles.sort((t1, t2) => t1.index - t2.index);
...
const canGameContinue = (grid: Cell[][], tiles: Tile[]) => {
  ...
  const dirs = [DIRECTION_MAP.Left, DIRECTION_MAP.Right, DIRECTION_MAP.Up, DIRECTION_MAP.Down];
...
const mergeAndCreateNewTiles = (grid: Cell[][]) => {
  ...
  const newTiles = createNewTilesInEmptyCells(emptyCells, Math.ceil((rows * cols) / 16));
...
const resetGameBoard = (rows: number, cols: number) => {
  ...
  const newTiles = createNewTilesInEmptyCells(emptyCells, Math.ceil((rows * cols) / 8));
...
const useGameBoard = ({ rows, cols, pause, gameStatus, setGameStatus, addScore }: GameBoardParams) => {
  ...
  const onMove = useCallback(
    (dir: Vector) => {
      if (pendingStackRef.current && !pauseRef.current) {
        const { tiles: newTiles, moveStack, grid } = moveInDirection(gridRef.current, dir);
  ...
  useEffect(() => {
    if (!moving) {
      const { tiles: newTiles, score, grid } = mergeAndCreateNewTiles(gridRef.current);
      gridRef.current = grid;
  ...
  useEffect(() => {
    if (gameStatus === "restart") {
      ...
      setGameStatus("win");
    } else if (gameStatus !== "lost" && !canGameContinue(gridRef.current, tiles)) {
      setGameStatus("lost");
    }
